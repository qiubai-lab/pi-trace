...
describe("Conversation", () => {
  it("merges one tool invocation across pages into one source-linked block", async () => {
    ...
    const canvas = screen.getByLabelText("Conversation canvas");
    Object.defineProperty(canvas, "scrollTop", { value: 200, writable: true });
    ...
    fireEvent.click(screen.getByRole("button", { name: "Continue to later context" }));
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "Close Event detail" })).toHaveFocus());
    fireEvent.keyDown(document, { key: "Escape" });
    await waitFor(() => expect(screen.queryByRole("dialog", { name: "Event detail" })).not.toBeInTheDocument());
    const source = screen.getByRole("button", { name: /read.*completed/i });
